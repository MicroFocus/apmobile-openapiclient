package com.hpe.apppulse.openapi.apppulseopenapi;

/**
 * Created by Meir Ron on 11/12/2015.
 */
public class constants {

     public static final String APPPULSE_URL_PREFIX="https://apppulse-mobile.saas.hpe.com";
//    public static final String APPPULSE_URL_PREFIX="https://apppulse-mobile.saas.hp.com";
    public static final String OPEN_API_VERSION = "v1";
    public static final String NO_ANSWER = "";
}
